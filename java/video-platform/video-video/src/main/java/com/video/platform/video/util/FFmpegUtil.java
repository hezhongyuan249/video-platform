package com.video.platform.video.util;
...
public class FFmpegUtil {
    ...
    private static void initFFmpegPath() {
        ...
        System.out.println("[FFmpegUtil] 使用 FFprobe 路径: " + FFPROBE_PATH);
        System.out.println("[FFmpegUtil] 使用 FFmpeg 路径: " + FFMPEG_PATH);
    }
    ...
    public Integer getVideoDuration(String videoPath) {
        ...
        try {
            ...
            System.out.println("[FFmpegUtil] 执行命令: " + String.join(" ", command));
            ...
            if (exitCode != 0) {
                System.err.println("[FFmpegUtil] FFprobe获取时长失败，exitCode: " + exitCode + ", output: " + result);
                return getVideoDurationFallback(videoPath);
            }

            if (result.isEmpty()) {
                System.err.println("[FFmpegUtil] FFprobe返回空结果");
                return getVideoDurationFallback(videoPath);
            }
            ...
            try {
                ...
                System.out.println("[FFmpegUtil] 获取到视频时长: " + duration + "秒");
                return duration;
            } catch (NumberFormatException e) {
                System.err.println("[FFmpegUtil] 解析时长失败: " + result);
                return getVideoDurationFallback(videoPath);
            }

        } catch (Exception e) {
            System.err.println("[FFmpegUtil] 获取视频时长失败: " + e.getMessage());
            e.printStackTrace();
            return getVideoDurationFallback(videoPath);
        }
    }
    ...
    private Integer getVideoDurationFallback(String videoPath) {
        try {
            ...
            System.out.println("[FFmpegUtil] Fallback执行命令: " + String.join(" ", command));
            ...
            if (matcher.find()) {
                ...
                System.out.println("[FFmpegUtil] Fallback获取到视频时长: " + duration + "秒");
                return duration;
            }

        } catch (Exception e) {
            System.err.println("[FFmpegUtil] Fallback获取视频时长失败: " + e.getMessage());
        }
        return 0;
    }
                ...
                while ((line = reader.readLine()) != null) {
                    if (progress != null && line.contains("time=")) {
                        try {
                            ...
                            if (currentTime > 0) {
                                int percent = (int) ((currentTime * 100.0) / totalDuration);
